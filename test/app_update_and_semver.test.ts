...
async function runSemverAndAutoUpdateTests() {
  ...
  console.log('\n--- 2. Semantic Version Comparison (compareSemver & isNewerVersion) ---');
  {
    ...
    // Default currentVersion (APP_VERSION = '3.5.0') & False-Positive Prevention
    assertEqual(APP_VERSION, '3.5.0', 'APP_VERSION constant is exactly 3.5.0');
    assert(!isNewerVersion('3.5.0'), 'Same version 3.5.0 does not trigger update');
    ...
    assert(!isNewerVersion('v3.5.0-build-55'), 'Remote build tag does NOT trigger false-positive update for official 3.5.0 users');
    assert(!isNewerVersion('v3.5.0-build-99'), 'Remote build 99 does NOT trigger update for official 3.5.0 release');
    assert(isNewerVersion('3.5.1'), 'Newer patch 3.5.1 triggers update for 3.5.0');
    assert(isNewerVersion('3.6.0'), 'Newer minor 3.6.0 triggers update for 3.5.0');
    assert(isNewerVersion('4.0.0'), 'Newer major 4.0.0 triggers update for 3.5.0');
    assert(isNewerVersion('v3.5.1-build-1'), 'Newer patch with build tag triggers update for 3.5.0');
    assert(isNewerVersion('v3.5.0', '3.5.0-build-54'), 'Official 3.5.0 triggers upgrade for test build 54 user');
  }
  ...
  console.log('\n--- 4. AppUpdateService Mock Tests & Ignored Versions ---');
  {
    ...
    assertEqual(AppUpdateService.getCurrentVersion(), '3.5.0', 'Current version is 3.5.0');
    ...
    assertEqual(state.ignoredVersion, '3.5.0', 'Ignored version persisted correctly');
