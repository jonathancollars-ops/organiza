...
async function runSemverAndAutoUpdateTests() {
  ...
  console.log('\n--- 2. Semantic Version Comparison (compareSemver & isNewerVersion) ---');
  {
    ...
    // Default currentVersion (APP_VERSION = '3.4.2') & False-Positive Prevention
    assertEqual(APP_VERSION, '3.4.2', 'APP_VERSION constant is exactly 3.4.2');
    assert(!isNewerVersion('3.4.2'), 'Same version 3.4.2 does not trigger update');
    ...
    assert(!isNewerVersion('v3.4.2-build-55'), 'Remote build tag does NOT trigger false-positive update for official 3.4.2 users');
    assert(!isNewerVersion('v3.4.2-build-99'), 'Remote build 99 does NOT trigger update for official 3.4.2 release');
    assert(isNewerVersion('3.4.3'), 'Newer patch 3.4.3 triggers update for 3.4.2');
    assert(isNewerVersion('3.5.0'), 'Newer minor 3.5.0 triggers update for 3.4.2');
    assert(isNewerVersion('4.0.0'), 'Newer major 4.0.0 triggers update for 3.4.2');
    assert(isNewerVersion('v3.4.3-build-1'), 'Newer patch with build tag triggers update for 3.4.2');
    assert(isNewerVersion('v3.4.2', '3.4.2-build-54'), 'Official 3.4.2 triggers upgrade for test build 54 user');
  }
  ...
  console.log('\n--- 4. AppUpdateService Mock Tests & Ignored Versions ---');
  {
    ...
    assertEqual(AppUpdateService.getCurrentVersion(), '3.4.2', 'Current version is 3.4.2');

    // State persistence & ignore version
    await AppUpdateService.ignoreVersion('3.4.2');
    const state = await AppUpdateService.getUpdateState();
    assertEqual(state.ignoredVersion, '3.4.2', 'Ignored version persisted correctly');
