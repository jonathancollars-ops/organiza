...
async function runAllChallengerProbes() {
  ...
  await runSection('AppUpdateService SemVer, Remote API & Throttling Resilience', async () => {
    ...
    (globalThis as any).fetch = async () => ({
      ...
      json: async () => ({
        tag_name: 'v3.6.0',
        name: 'Lumen v3.6.0 Release',
        body: 'Awesome new features',
        html_url: 'https://github.com/jonathancollars-ops/organiza/releases/tag/v3.6.0',
        assets: [
          { name: 'source.zip', browser_download_url: 'https://example.com/source.zip' },
          { name: 'lumen-v3.6.0.apk', browser_download_url: 'https://example.com/lumen.apk' }
        ]
      })
    });

    const checkSuccess = await AppUpdateService.checkForUpdates(true);
    assert(checkSuccess !== null && checkSuccess.hasUpdate === true, 'checkForUpdates detects newer v3.6.0 release');
    assert(checkSuccess?.latestVersion === '3.6.0', 'latestVersion parsed correctly');
