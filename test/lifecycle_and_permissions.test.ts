  ...
  await testSection('Network Timeout & Resilience in AppUpdateService', async () => {
    ...
    try {
      ...
      (globalThis as any).fetch = async () => ({
        ...
        json: async () => ({
          tag_name: 'v3.6.1',
          name: 'Lumen v3.6.1 - Super Atualização',
          body: 'Novas funcionalidades e estabilidade.',
          assets: [
            {
              name: 'lumen-v3.6.1-standalone.apk',
              browser_download_url: 'https://github.com/jonathancollars-ops/organiza/releases/download/v3.6.1/lumen.apk',
            }
          ],
      ...
      assert(updateInfo?.latestVersion === '3.6.1', 'updateInfo.latestVersion is 3.5.0');
