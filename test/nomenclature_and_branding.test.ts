...
async function runNomenclatureTests() {
  ...
  await test('1. package.json nomenclature & version alignment', () => {
    ...
    assert(pkg.version === '3.2.1', `package.json "version" is "3.2.1" (got: "${pkg.version}")`);
  });
  ...
  await test('2. app.json nomenclature & version alignment', () => {
    ...
    assert(appJson.expo.version === '3.2.1', `app.json expo.version is "3.2.1" (got: "${appJson.expo.version}")`);
  });
