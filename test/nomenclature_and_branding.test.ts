...
async function runNomenclatureTests() {
  ...
  // Test 10: Version 3.5.0 alignment across version.ts, package.json, app.json, and AppUpdateService
  await test('10. Version 3.5.0 alignment across version.ts, package.json, app.json and AppUpdateService', () => {
    const pkgPath = path.join(projectRoot, 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    const appJsonPath = path.join(projectRoot, 'app.json');
    const appJson = JSON.parse(fs.readFileSync(appJsonPath, 'utf8'));

    assert(APP_VERSION === '3.5.0', `APP_VERSION is "3.5.0" (got: "${APP_VERSION}")`);
    assert(pkg.version === '3.5.0', `package.json version is "3.5.0" (got: "${pkg.version}")`);
    assert(appJson.expo.version === '3.5.0', `app.json expo.version is "3.5.0" (got: "${appJson.expo.version}")`);
    assert(AppUpdateService.getCurrentVersion() === '3.5.0', `AppUpdateService.getCurrentVersion() is "3.5.0" (got: "${AppUpdateService.getCurrentVersion()}")`);
  });
