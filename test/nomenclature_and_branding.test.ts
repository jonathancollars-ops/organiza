...
async function runNomenclatureTests() {
  ...
  await test('3. Android strings.xml app_name alignment', () => {
    ...
    if (!fs.existsSync(stringsPath)) {
      console.log('  ℹ️  strings.xml not yet generated (expected in CI pre-prebuild). Skipping.');
      return;
    }
  ...
  await test('9. Project-wide metadata consistency across manifests and components', () => {
    // Note: android/strings.xml is excluded — it's generated by expo prebuild (not available pre-build)
    const filesToAudit = [
    ...
    // Also validate strings.xml if it exists (local dev / post-prebuild)
    const stringsXml = path.join(projectRoot, 'android', 'app', 'src', 'main', 'res', 'values', 'strings.xml');
    if (fs.existsSync(stringsXml)) {
      filesToAudit.push(stringsXml);
    }
  ...
  // Test 10: Version 3.5.1 alignment across version.ts, package.json, app.json, and AppUpdateService
  await test('10. Version 3.5.1 alignment across version.ts, package.json, app.json and AppUpdateService', () => {
    const pkgPath = path.join(projectRoot, 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    const appJsonPath = path.join(projectRoot, 'app.json');
    const appJson = JSON.parse(fs.readFileSync(appJsonPath, 'utf8'));

    assert(APP_VERSION === '3.5.1', `APP_VERSION is "3.5.1" (got: "${APP_VERSION}")`);
    assert(pkg.version === '3.5.1', `package.json version is "3.5.1" (got: "${pkg.version}")`);
    assert(appJson.expo.version === '3.5.1', `app.json expo.version is "3.5.1" (got: "${appJson.expo.version}")`);
    assert(AppUpdateService.getCurrentVersion() === '3.5.1', `AppUpdateService.getCurrentVersion() is "3.5.1" (got: "${AppUpdateService.getCurrentVersion()}")`);
  });
