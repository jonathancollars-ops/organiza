...
async function runSecurityTests() {
  ...
  await test('AppUpdateService.checkForUpdates sanitizes GitHub release data and falls back when no APK asset exists', async () => {
    ...
    globalThis.fetch = async () => ({
      ok: true,
      json: async () => ({
        tag_name: 'v3.4.3-build-12',
        name: 'Release 3.4.3 <script>alert("xss")</script>',
        body: 'Notas de atualização <b>com tags</b> e <iframe src="evil.com"></iframe>',
        html_url: 'https://github.com/jonathancollars-ops/organiza/releases/tag/v3.4.3',
        assets: [
    ...
    assert(info!.latestVersion === '3.4.3-build-12', 'Version with build extracted');
    ...
    assert(info!.downloadUrl === 'https://github.com/jonathancollars-ops/organiza/releases/tag/v3.4.3', 'Fallback to release html_url because no .apk asset exists');
